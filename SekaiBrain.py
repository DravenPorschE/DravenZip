...
def create_recording_indicator():
    ...
    # Use system-specific transparent color
    ...
    recording_indicator.place(x=SCREEN_WIDTH - indicator_width - 10, y=10)
...
def show_recording_indicator():
    ...
    # Update canvas background to match current view
    ...
    recording_indicator.config(bg=bg_color)
...
def start_flashing_indicator():
    ...
    bg_color = recording_indicator.cget('bg')
    ...
    # Draw red dot (●)
    recording_indicator.create_text(15, 15, text="●", 
                                   fill=color, font=("Arial", 20),
                                   anchor="center")
    
    # Draw "REC" text
    recording_indicator.create_text(45, 15, text="REC", 
...
happy_results = [
    "happy_showing_results_1.wav",
    "happy_showing_results_1.wav",
    "happy_showing_results_1.wav"
]

angry_results = [
    "angry_showing_results_1.wav",
    "angry_showing_results_2.wav",
    "angry_showing_results_3.wav"
]

def play_mood_audio(current_mood):
    """Plays a random audio file based on the current mood"""
    if current_mood == "happy":
        audio_file = random.choice(happy_results)
    elif current_mood == "angry":
        audio_file = random.choice(angry_results)
    else:
        return  # no audio for other moods

    if os.path.exists(audio_file):
        try:
            subprocess.run(["aplay", "-D", "plughw:1,0", audio_file], check=True)
        except Exception as e:
...
def send_audio(recording_file):
    ...
    if intent in ["display_calendar", "display_weather"]:
        ...
        if intent == "display_calendar":
            ...
            if current_mood == "happy":
                play_mood_audio("happy")
            if current_mood == "angry":
                play_mood_audio("angry")
